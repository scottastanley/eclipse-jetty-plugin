...
/**
 * Implemation of the {@link ServerAdapter} for Jetty 11
 * 
    ...
    protected Collection<String> getClassPathDescription(Collection<String> classPath, Handler... handlers)
    {
        if (handlers != null)
        {
            for (Handler handler : handlers)
            {
                ...
                {
                	List<Resource> resources = ((WebAppContext) handler).getExtraClasspath();
                	if (resources != null) {
                		List<String> extraClasspaths = new ArrayList<String>();
                    	for (Resource r : resources) {
                    		extraClasspaths.add(r.getURI().getPath());
                    	}
                    	
                    	Collections.addAll(classPath, extraClasspaths.toArray(new String[0]));
                	}
                }
            }
        }
